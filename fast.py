...
async def forward_message(client, group, last_message, session_name):
    try:
...
async def login_and_forward(api_id, api_hash, phone_number, session_name):
    ...
    # Ask for delay after all groups
    delay_between_rounds = int(input(f"Enter the delay (in seconds) before the next round for {session_name}: "))

    for round_num in range(1, repeat_count + 1):
        print(f"\nStarting round {round_num} of forwarding messages to all groups for {session_name}.")

        # Fetch the last message each round
        ...
        tasks = []  # Create a list to hold the forwarding tasks

        async for dialog in client.iter_dialogs():
            if dialog.is_group:
                group = dialog.entity
                tasks.append(forward_message(client, group, last_message, session_name))
